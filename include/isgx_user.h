...
/*---------------------tmac patch----------------------*/
//For migration:
#define SGX_IOC_ENCLAVE_INT \
	_IOW(SGX_MAGIC, 0x100, struct sgx_enclave_create)
//For acceleration:
#define SGX_IOC_ENCLAVE_ADD_MULTI_PAGE \
	_IOW(SGX_MAGIC, 0x101, struct sgx_enclave_add_multi_page)

struct sgx_enclave_add_multi_page {
	__u64	addr;
	__u64	src;
	__u64	secinfo;
	__u16	mrmask;
