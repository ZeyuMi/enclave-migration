...
SLOG("RemoveMemArray: 0x%lx, 0x%lx\n", true_addr, adj_addr);
...
SLOG("In AddMemArray: mem_array_ents is %d, true_add is 0x%lx, adj_addr is 0x%lx\n", \
	 mem_array_ents, true_addr, adj_addr);
mem_array[0][mem_array_ents]=true_addr;
mem_array[1][mem_array_ents]=adj_addr;
mem_array_ents++;
SLOG("In AddMemArray: mem_array_ents is %d, addr of it is 0x%lx\n", mem_array_ents, &mem_array_ents);
return(0);
}
...
SLOG("In RemoveMemArray: mem_array_ents %d, adj_addr 0x%lx\n", mem_array_ents, adj_addr);
