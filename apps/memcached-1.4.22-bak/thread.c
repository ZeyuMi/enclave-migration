...
static void wait_for_thread_registration(int nthreads) {
	...
	//for(i = 0; i < 100000; ++i);
	//init_count += 1;
	if(SGXDEBUG) printf("[Memcached]****** init_count: %d, nthreads: %d\n", init_count, nthreads);
...
static void *worker_libevent(void *arg) {
	...
	if(SGXDEBUG) printf("[Memcached] thread file:%s, line: %d\n", __FILE__, __LINE__);
...
void memcached_thread_init(int nthreads, struct event_base *main_base) {
	...
	if(SGXDEBUG) printf("[Memcached] thread is 0x%lx\n", (long)dispatcher_thread.thread_id);
