...
//TODO: tmac
extern long ocall_syscall6(long n, long a1, long a2, long a3, long a4, long a5, long a6);
...
conn *conn_new(const int sfd, enum conn_states init_state,
    ...
    if (transport == tcp_transport && init_state == conn_new_cmd) {
		if(SGXDEBUG)
			printf("sfd: %d, c->ra addr 0x%lx, size 0x%x\n", sfd, (long)&c->request_addr, 
																	c->request_addr_size);
        if (getpeername(sfd, (struct sockaddr *) &c->request_addr,
                        &c->request_addr_size)) {
			if(SGXDEBUG) printf("getpeername failed\n");
            perror("getpeername");
...
static inline void process_get_command(conn *c, token_t *tokens, size_t ntokens, bool return_cas) {
    ...
    */
    if (key_token->value != NULL || add_iov(c, "END\r\n", 5) != 0
		...
		if(SGXDEBUG) printf("-------[Memcached] file: %s, line:%d\n", __FILE__, __LINE__);
        conn_set_state(c, conn_mwrite);
        c->msgcurr = 0;
    }
}
...
static enum transmit_result transmit(conn *c) {
    ...
    if (c->msgcurr < c->msgused) {
		...
		if(SGXDEBUG)
		{
			...
			vecsz = m->msg_iovlen;
			printf("***** msg addr 0x%lx, iovec size %d:\n", (unsigned long)m, vecsz);
			printf("msg_name: 0x%lx, msg_iov: 0x%lx, msg_control: 0x%lx\n",
				          m->msg_name, m->msg_iov, m->msg_control);               
			for(idx = 0; idx < vecsz; ++idx)
			{
				vec = m->msg_iov + idx;
				printf("iov[%d] addr is 0x%lx, its base 0x%lx\n", idx, (unsigned long)vec, vec->iov_base);
				for(i = 0; i < vec->iov_len; ++i)
					printf("NO.%d: %c\n", i, *(char*)(vec->iov_base + i));	
			}

			asm("mov %%rsp, %0":"=g"(stack)::);
			printf("current stack is 0x%lx\n", stack);
		}
		*/

        res = sendmsg(c->sfd, m, 0);
		//res = ocall_syscall6(46, c->sfd, m, 0, 0, 0, 0);
...
static void drive_machine(conn *c) {
    ...
    while (!stop) {
        ...
        case conn_nread:
			...
			if(SGXDEBUG) printf("[Memcached] file: %s, line:%d\n", __FILE__, __LINE__);
			if(SGXDEBUG) printf("[Memcached] errno is %d, addr of errno is 0x%lx, EAGAIN is %d, res is %d\n", errno, 
								(unsigned long)&errno, EAGAIN, res);
            if (res == -1 && (errno == EAGAIN || errno == EWOULDBLOCK)) {
				if(SGXDEBUG) printf("[Memcached] file: %s, line:%d\n", __FILE__, __LINE__);
                if (!update_event(c, EV_READ | EV_PERSIST)) {
				if(SGXDEBUG) printf("[Memcached] file: %s, line:%d\n", __FILE__, __LINE__);
...
int main (int argc, char **argv) {
	...
	if(SGXDEBUG) printf("[Memcached] func:%s line: %d\n", __func__, __LINE__);
